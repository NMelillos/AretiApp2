...
def main():
    ...
    assert_equal("whole-word Income and existing Charity rows included", scoped["id"].tolist(), [1, 2, 3, 4, 5, 6, 7, 11, 12, 13])
    assert_close("January Income", monthly["Income"][months[0]], 1000)
    assert_close("February Income follows category or subcategory", monthly["Income"][months[1]], 3749)
    assert_close("February Charity keeps statement sign", monthly["Charity"][months[1]], -50)
    assert_close("Income cumulative from January", cumulative["Income"][months[1]], 4749)
    ...
    assert_equal("Reporting Group alone does not qualify Projects", 9 in scoped["id"].tolist(), False)
    assert_equal("Reporting Group alone does not qualify deposits", 10 in scoped["id"].tolist(), False)
    assert_equal("whole-word income outside Income group included", 11 in scoped["id"].tolist(), True)
    assert_equal("Category Income outside Income group included", 13 in scoped["id"].tolist(), True)
